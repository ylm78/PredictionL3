...
soup = BeautifulSoup(data.text, features="html.parser") #On transforme le contenu de notre page en objet BeautifulSoup
...
soup = BeautifulSoup(data.text, features="lxml") #soup devient un objet BeautifulSoup que l'on peut scrapper
...
for year in years:
    ...
    for team_url in team_urls:
            ...
            soup = BeautifulSoup(data.text, features="lxml") #On creer un objet beautifulSoup
...
# In[193]:
def get_team_names():
    return match_df['team'].unique().tolist()

# In[194]
def get_opp_names():
    return match_df['opponent'].unique().tolist()
# In[195]
def get_date():
    return match_df['opponent'].unique().tolist()